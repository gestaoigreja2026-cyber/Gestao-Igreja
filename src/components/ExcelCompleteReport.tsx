...
export function ExcelCompleteReportButton({ disabled }: { disabled?: boolean }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      d1.forEach((row: any, r: number) => { const bg = r % 2 === 0 ? C.WHITE : C.GRAY; row.forEach((v: any, c: number) => { const val = v === undefined || v === null ? '' : v; if (c === 7) ws1[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, bStyle(bg, val === 'Ativo' ? C.GREEN : C.RED)); else if (c === 3) ws1[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, cStyle(bg, true), 'n'); else ws1[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, cStyle(bg)); }); });
      ...
      const d3 = members.map((m: any, i: number) => [i + 1, String(m.name || ''), formatDateSafe(m.birth_date), String(m.phone || ''), String(m.email || ''), String(m.address || ''), String(m.neighborhood || ''), m.baptized ? 'Sim' : 'Não', formatDateSafe(m.baptism_date), m.status === 'active' ? 'Membro' : m.status === 'visitor' ? 'Visitante' : m.status === 'child' ? 'Criança' : 'Inativo', String(m.ministry?.name || ''), String(m.cell?.name || ''), formatDateSafe(m.created_at), String(m.notes || '')]);
      ...
      d3.forEach((row: any, r: number) => { const bg = r % 2 === 0 ? C.WHITE : C.GRAY; const st: any = { 'Membro': C.GREEN, 'Visitante': C.GOLD, 'Criança': C.PURPLE, 'Inativo': C.RED }; row.forEach((v: any, c: number) => { const val = v === undefined || v === null ? '' : v; if (c === 9) ws3[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, bStyle(bg, st[val] || C.TEXT)); else if (c === 0) ws3[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, cStyle(bg, true), 'n'); else ws3[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, cStyle(bg, c === 7 || c === 8)); }); });
      ...
      const d8 = assets.map((a: any) => [String(a.code || ''), String(a.name || ''), String(a.category || ''), formatDateSafe(a.acquisition_date), String(a.invoice_number || ''), a.value || 0, a.current_value || 0, a.status || 'Ativo', String(a.location || ''), String(a.responsible?.name || ''), formatDateSafe(a.next_maintenance), String(a.notes || '')]);
      ...
      const d9 = prayers.map((p: any, i: number) => [i + 1, formatDateSafe(p.created_at), String(p.requester_name || ''), String(p.requester_contact || ''), String(p.request || ''), String(p.category || ''), p.status === 'open' ? 'Aberto' : p.status === 'in_progress' ? 'Em oração' : p.status === 'answered' ? 'Respondido' : 'Arquivado', String(p.responsible?.name || ''), formatDateSafe(p.response_date), String(p.testimony || '')]);
      ...
      d9.forEach((row: any, r: number) => { const bg = r % 2 === 0 ? C.WHITE : C.GRAY; const st: any = { 'Aberto': C.RED, 'Em oração': C.GOLD, 'Respondido': C.GREEN }; row.forEach((v: any, c: number) => { const val = v === undefined || v === null ? '' : v; if (c === 6) ws9[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, bStyle(bg, st[val] || C.TEXT)); else if (c === 0) ws9[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, cStyle(bg, true), 'n'); else ws9[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, cStyle(bg, c === 1 || c === 6 || c === 8)); }); });
      ...
      const ws10 = XLSX.utils.aoa_to_sheet([[], [], [], h10, ...d10]);
      ws10['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 14 } }, { s: { r: 1, c: 0 }, e: { r: 1, c: 14 } }];
      ws10['A1'] = sc('  ✝  CONSOLIDAÇÃO', tStyle(C.ORANGE));
      ws10['A2'] = sc('   Acompanhamento de novos convertidos - 4 semanas', sStyle(C.ORANG_L));
      h10.forEach((h, i) => ws10[XLSX.utils.encode_cell({ r: 3, c: i })] = sc(h, hStyle(C.ORANGE)));
      d10.forEach((row: any, r: number) => {
        const bg = r % 2 === 0 ? C.WHITE : C.GRAY;
        row.forEach((v: any, c: number) => {
          const val = v === undefined || v === null ? '' : v;
          if (c === 0) {
            ws10[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, cStyle(bg, true), 'n');
          } else if (c === 13 && val) {
            // Data de batismo destacada
            ws10[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, { font: { name: 'Arial', bold: true, sz: 10, color: { rgb: C.TEAL } }, fill: { patternType: 'solid', fgColor: { rgb: C.TEAL_L } }, alignment: { horizontal: 'center', vertical: 'center' } });
          } else {
            ws10[XLSX.utils.encode_cell({ r: r + 4, c })] = sc(val, cStyle(bg, c >= 6 && c <= 12 && c % 2 === 0)); // Centraliza datas
          }
        });
      });
